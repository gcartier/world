...
// jazz
// - connection state (probably sock + tcp_sock)
// - packet state (probably tcp_skb_cb)
// - rate sample (probably rate_sample)
...
#define do_div(x, y) x = x / y // bazoo
#define div_u64(x, y) x / y // bazoo

#define cmpxchg(x, y, z) x // bazoo

#define likely(x) x // bazoo
#define unlikely(x) x // bazoo

#define before(x, y) ((x) < (y)) // bazoo
#define after(x, y) ((x) > (y)) // bazoo

#define READ_ONCE(x) x // bazoo

#define WARN_ONCE(condition, format...) 0 // bazoo
#define WARN_ON_ONCE(x) x // bazoo

#define BUILD_BUG_ON(x) // bazoo
...
static u32 max(u32 x, u32 y) {
	// bazoo
	return x;
}

#define min_t(type, x, y) x // bazoo

#define max_t(type, x, y) x // bazoo
